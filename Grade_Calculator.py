        ...
        try:
            ...
            if self.optionChoose == "cem":
                if float(self.final_grade_weight_input.text()) != 0:
                    result = (float(self.final_input.text()) - mark_before_exam) / float(self.final_grade_weight_input.text())/100
                result = (float(self.final_input.text()) - mark_before_exam)
                self.result_label.setText(f"You need to get at least {result:.2f}% on you exam inorder to achieve your expectation.")
            elif self.optionChoose == "cfm":
                result = mark_before_exam + float(self.final_input.text()) * float(self.final_grade_weight_input.text())/100
                self.result_label.setText(f"You final grade is {result:.2f}%")
                
        except ValueError:
            self.show_error_message("Invalid input", "Please re-check the grades and weight you entered!")
